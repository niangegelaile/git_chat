...
public class Client {
	...
	public static final int STATE_OPEN=1;//socket打开
	public static final int STATE_CLOSE=1<<1;//socket关闭
	public static final int STATE_CONNECT_START=1<<2;//开始连接server
	public static final int STATE_CONNECT_SUCCESS=1<<3;//连接成功
	public static final int STATE_CONNECT_FAILED=1<<4;//连接失败
	public static final int STATE_CONNECT_WAIT=1<<5;//等待连接
			...
			try {
					while(state!=STATE_CLOSE)
					{
						try {
							...
							setState(STATE_CONNECT_SUCCESS);
						} catch (Exception e) {
							e.printStackTrace();
...
//Log.v(TAG,"Rec :End");
	...
	private void setState(int s){
		...
		if(respListener!=null){
			respListener.onConnectStatus(state);
		}
	}
